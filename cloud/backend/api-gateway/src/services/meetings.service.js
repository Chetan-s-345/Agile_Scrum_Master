...
function isValidDailyJoinUrl(value) {
  try {
    const url = new URL(String(value || '').trim());
    return url.protocol === 'https:' && /\.daily\.co$/i.test(url.hostname);
  } catch {
    return false;
  }
}
...
class MeetingsService {
  async createDailyRoom(meetingId, durationMinutes = 60) {
    const apiKey = String(process.env.DAILY_API_KEY || '').trim();
    if (!apiKey) {
      throw Object.assign(new Error('DAILY_API_KEY is not configured'), {
        statusCode: 500,
        code: 'DAILY_CONFIG_MISSING',
        detail: 'Set DAILY_API_KEY in the api-gateway environment.',
      });
    }

    const exp = Math.floor(Date.now() / 1000) + Number(durationMinutes || 60) * 60;
    const res = await fetch('https://api.daily.co/v1/rooms', {
      ...
      body: JSON.stringify({
        ...
        properties: { exp, enable_recording: false },
      }),
    });

    if (!res.ok) {
      const err = await res.json().catch(() => null);
      throw Object.assign(new Error(`Daily.co error: ${err?.error || res.status}`), {
        statusCode: 502,
        code: 'DAILY_ROOM_CREATE_FAILED',
        detail: String(err?.error || err?.info || `Daily room creation failed with status ${res.status}`),
      });
    }

    const room = await res.json();
    return { joinUrl: room.url, providerMeetingId: room.name };
  }
  ...
  async createWithPool({ orgPool, actorMemberId, orgId, payload }) {
    ...
    const provider = normalizeProvider(payload?.provider || 'daily');
    ...
    if (shouldCreateJoinUrl && provider === 'daily' && !joinUrl) {
    ...
    if (provider === 'daily' && joinUrl && !isValidDailyJoinUrl(joinUrl)) {
      throw Object.assign(new Error('joinUrl must be a valid Daily.co URL'), {
        statusCode: 400,
        code: 'DAILY_INVALID_JOIN_URL',
        detail: 'Expected an https://*.daily.co/... URL when provider is daily.',
      });
    }
  ...
  async startMeeting(req, meetingId, payload) {
    ...
    let provider = normalizeProvider(meeting?.video_provider || payload?.provider || 'daily');
    ...
    if (!providerMeetingId && joinUrl) {
      providerMeetingId = `daily-${crypto.randomUUID().slice(0, 12)}`;
    }
  ...
  getHealth() {
    ...
    const dailyDomain = String(process.env.DAILY_DOMAIN || '').trim();

    return {
      status: 'ok',
      daily: {
        configured: Boolean(dailyApiKey),
        apiKeySet: Boolean(dailyApiKey),
        domainSet: Boolean(dailyDomain),
