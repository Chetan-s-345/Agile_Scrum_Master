/*
  Adds task parent-child support to all tenant org databases.

  Usage:
    node db/migrations/add_task_subtasks.js
...
async function hasConstraint(pool, constraintName) {
  const resp = await pool.query(
    `SELECT 1
     FROM pg_constraint
     WHERE conname = $1
...
async function migrateTenant(orgId, connectionString) {
  ...
  try {
    ...
    await pool.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS parent_task_id UUID`);
    await pool.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS is_subtask BOOLEAN DEFAULT FALSE`);
    await pool.query(`UPDATE tasks SET is_subtask = FALSE WHERE is_subtask IS NULL`);

    const fkName = 'tasks_parent_task_id_fkey';
    const hasFk = await hasConstraint(pool, fkName);
    if (!hasFk) {
      await pool.query(
        `ALTER TABLE tasks
