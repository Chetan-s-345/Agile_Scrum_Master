...
export async function POST(_request: Request, { params }: { params: Promise<{ sprintId: string }> }) {
  const token = await getAuthToken();
  if (!token) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { sprintId } = await params;
  const gatewayResp = await fetch(
    `${getApiGatewayBaseUrl()}/api/v1/sprints/${encodeURIComponent(sprintId)}/schedule-meetings`,
    {
      method: "POST",
