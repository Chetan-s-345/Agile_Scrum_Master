/*
  Adds tasks.jira_synced to all tenant org databases.

  Usage:
    node scripts/migrate-add-jira-synced.js
...
function normalizeTenantDbConnectionString(connectionString) {
  // Keep behavior aligned with runtime code, but avoid importing app modules.
  // Minimal normalization: trim and return.
...
async function migrateOrg(orgId, connStr) {
  ...
  try {
    ...
    // If tasks table doesn't exist, skip.
    const tasksExists = await orgPool.query(
      `SELECT 1
    ...
    if (!tasksExists.rows.length) {
      await orgPool.query('ROLLBACK');
      return { orgId, ok: false, skipped: true, reason: 'tasks_table_missing' };
    }

    await orgPool.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS jira_synced BOOLEAN DEFAULT TRUE`);

    // Backfill nulls (if column existed but was nullable from some prior manual change).
    await orgPool.query(`UPDATE tasks SET jira_synced = TRUE WHERE jira_synced IS NULL`);
