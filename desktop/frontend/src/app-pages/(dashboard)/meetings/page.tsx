...
import { usePathname, useRouter } from "@/next-shims/navigation";
import { CalendarDays, FileText, Loader2, NotebookPen, PlayCircle, RefreshCw, Sparkles, Upload, Users } from "lucide-react";
...
function normalizeError(payload: unknown, fallback: string): string {
  const maybe = payload as { error?: unknown } | null;
...
function MeetingsPageContent() {
  ...
  const refreshMeetingDetail = async (meetingId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/meetings/${encodeURIComponent(meetingId)}`, { cache: "no-store" });
  ...
  const refreshTranscripts = async (meetingId: string) => {
    ...
    try {
      const response = await fetch(`/api/meetings/${encodeURIComponent(meetingId)}/transcripts`, { cache: "no-store" });
  ...
  const startMeeting = async () => {
    ...
    try {
      const response = await fetch(`/api/meetings/${encodeURIComponent(selectedMeetingId)}/start`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider: "google_meet" }),
      });
      const payload = (await response.json().catch(() => null)) as MeetingDetailResponse | null;
  ...
  return (
    <div className="space-y-4 p-4 md:p-5">
      {DESKTOP_READ_ONLY ? (
        <div className="rounded-md border border-[#7a5b1f] bg-[#2f2411] px-3 py-2 text-xs text-[#f3d39b]">
          Desktop v1 is read-only. Create, update, start meeting, transcript upload, and AI write actions are disabled.
        </div>
      ) : null}
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-3">
            ...
            <button
              ...
              disabled={DESKTOP_READ_ONLY || creating || !newTitle.trim() || !newDateTime}
              ...
              <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <select
                    ...
                    disabled={DESKTOP_READ_ONLY || savingStatus}
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <button
                    ...
                    disabled={DESKTOP_READ_ONLY || startingMeeting}
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={DESKTOP_READ_ONLY || savingDescription}
                        className="inline-flex h-8 items-center gap-1 rounded-md border border-[var(--border)] px-2 text-xs"
                      >
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                  ...
                  <button
                    ...
                    disabled={DESKTOP_READ_ONLY || savingNote || !noteDraft.trim()}
              ...
              <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    disabled={DESKTOP_READ_ONLY || savingAttendees || !selectedMeetingId}
                    className="inline-flex h-8 items-center gap-1 rounded-md border border-[var(--border-strong)] px-2 text-xs disabled:opacity-60"
                  >
              ...
              <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    disabled={DESKTOP_READ_ONLY || summarizingMeeting}
                    className="inline-flex h-8 items-center gap-1 rounded-md border border-[var(--border-strong)] px-2 text-xs disabled:opacity-60"
                  >
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={DESKTOP_READ_ONLY || savingTranscript || !transcriptDraft.trim()}
                    className="inline-flex h-8 items-center gap-1 rounded-md border border-[var(--border-strong)] px-2 text-xs disabled:opacity-60"
                  >
